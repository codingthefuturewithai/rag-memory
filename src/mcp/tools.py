...
_analysis_tokens: Dict[str, Dict] = {}  # {token: {"domains": [list], "timestamp": float}}
TOKEN_EXPIRY_SECONDS = 14400  # 4 hours


def store_analysis_token(token: str, domains: List[str]) -> None:
    """
    Store analysis token with domains and timestamp.

    Args:
        token: Analysis token from analyze_website()
        domains: List of domains found in sitemap (or [base_url] if no sitemap)
    """
    _analysis_tokens[token] = {
        "domains": domains,
        "timestamp": time.time()
    }
...
def validate_token(token: str, ingest_url: str) -> Tuple[bool, Optional[str]]:
    """
    Validate token exists, is not expired, and covers the ingest domain.

    Token is reusable - not consumed on validation. Expires after 4 hours only.
    Uses root domain matching to handle multi-subdomain sites.

    Args:
        token: Analysis token from agent
        ingest_url: URL agent wants to crawl
    ...
    if token not in _analysis_tokens:
        return False, (
            "Invalid or expired analysis_token.\n\n"
            "Possible causes:\n"
            "- Token not generated by analyze_website() (fake token)\n"
            "- Token expired (4 hour lifetime)\n\n"
            "Solution: Run analyze_website() again to get a fresh token."
        )

    # Extract root domain from ingest URL
    from urllib.parse import urlparse
    ingest_netloc = urlparse(ingest_url).netloc
    ingest_root = extract_root_domain(ingest_netloc)

    # Check if ingest root matches any stored domain's root
    token_data = _analysis_tokens[token]
    stored_domains = token_data["domains"]

    for stored_domain in stored_domains:
        stored_root = extract_root_domain(stored_domain)
        if ingest_root == stored_root:
            return True, None

    # No match found
    return False, (
        f"Domain mismatch: Token covers {stored_domains}, "
        f"but you're trying to ingest {ingest_netloc}.\n\n"
        "Solution: Run analyze_website() for the target domain first."
    )


def extract_root_domain(netloc: str) -> str:
    """
    Extract root domain from netloc (handles subdomains).
    ...
    Args:
        netloc: Network location (domain with optional subdomain)
...
def _cleanup_expired_tokens() -> None:
    ...
    validate_token() is called. Tokens are reusable, so accumulation
    limited to abandoned analyses that expire after 4 hours.
    """
    now = time.time()
    expired = [
        token for token, data in _analysis_tokens.items()
        if now - data["timestamp"] > TOKEN_EXPIRY_SECONDS
    ]
...
def analyze_website_impl(
    ...
    try:
        ...
        if result.get("analysis_token") and result.get("domains"):
            store_analysis_token(result["analysis_token"], result["domains"])
            logger.debug(f"Stored analysis token for {base_url} with domains: {result['domains']}")
        ...
        if follow_links:
            if not analysis_token:
                raise ValueError(
                    ...
                    "REQUIRED WORKFLOW:\n"
                    "1. analyze_website(url) - understand site structure, get token\n"
                    "2. Review total_urls and pattern_stats\n"
                    "3. ingest_url(url, follow_links=True, analysis_token=<token>)\n\n"
                    "TOKEN BEHAVIOR:\n"
                    "- Tokens are REUSABLE (not consumed on use)\n"
                    "- Valid for 4 hours (supports multiple targeted crawls)\n"
                    "- Domain-scoped (only works for domains found in analysis)\n\n"
                    "SINGLE-PAGE CRAWLS:\n"
                    "- No token required for follow_links=False (bounded to 1 page)\n"
                    "- Use when you just want to ingest a single specific page"
                )

            # Validate token is legitimate and covers this URL's domain
            is_valid, error_msg = validate_token(analysis_token, url)
            if not is_valid:
