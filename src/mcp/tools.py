...
# ============================================================================
# Tokens are one-time use, expire after 4 hours
# Long expiry accounts for real-world usage: user reviews analysis, gets
# distracted, comes back hours later to approve crawl
_analysis_tokens: Dict[str, float] = {}  # {token: timestamp}
TOKEN_EXPIRY_SECONDS = 14400  # 4 hours


def store_analysis_token(token: str) -> None:
    """Store analysis token with current timestamp."""
    _analysis_tokens[token] = time.time()
    _cleanup_expired_tokens()


def validate_and_consume_token(token: str) -> bool:
    """
    Validate token exists and is not expired, then remove it (one-time use).

    Returns:
        True if valid, False if invalid/expired/already used
    """
    _cleanup_expired_tokens()

    if token not in _analysis_tokens:
        return False

    # Token exists - consume it (one-time use)
    del _analysis_tokens[token]
    return True


def _cleanup_expired_tokens() -> None:
    """
    Remove tokens older than TOKEN_EXPIRY_SECONDS.

    Lazy cleanup - only runs when store_analysis_token() or
    validate_and_consume_token() is called. Tokens are consumed on use,
    so accumulation is limited to abandoned analyses.
    """
    now = time.time()
    expired = [
        token for token, timestamp in _analysis_tokens.items()
...
def analyze_website_impl(
    ...
    Stores analysis_token server-side for validation in ingest_url.
    """
    try:
        result = analyze_website(base_url, timeout, include_url_lists, max_urls_per_pattern)

        # Store token server-side for validation (if token was generated)
        if result.get("analysis_token"):
            store_analysis_token(result["analysis_token"])
            logger.debug(f"Stored analysis token for {base_url}")
...
async def ingest_url_impl(
    ...
    try:
        ...
        if follow_links:
            ...
            # Validate token is legitimate (stored server-side by analyze_website)
            if not validate_and_consume_token(analysis_token):
                raise ValueError(
                    f"Invalid or expired analysis_token.\n\n"
                    f"Possible causes:\n"
                    f"- Token not generated by analyze_website() (fake token)\n"
                    f"- Token already used (tokens are one-time use)\n"
                    f"- Token expired (4 hour lifetime)\n\n"
                    f"Solution: Run analyze_website() again to get a fresh token."
                )
