# Test MCP Server - Quick Setup Guide
...
test_mcp_server-server --transport streamable-http --port 3001
...
echo '{"method": "tools/list"}' | test_mcp_server-server
```
...
# View logs
# macOS:
tail -f ~/Library/Logs/mcp-servers/test_mcp_server.log
# Linux:
tail -f ~/.local/state/mcp-servers/logs/test_mcp_server.log
# Windows PowerShell:
Get-Content -Wait "$env:LOCALAPPDATA\mcp-servers\logs\test_mcp_server.log"
```

### Transport Debugging
```bash
# Test stdio (default)
echo '{"method": "ping"}' | test_mcp_server-server
...
# Windows PowerShell:
Start-Job { test_mcp_server-server --transport sse --port 3001 }
...
# Windows PowerShell:
Start-Job { test_mcp_server-server --transport streamable-http --port 3001 }
...
- **MCP_SERVER_NAME**: Custom server name (default: "Test MCP Server")
...
2. ✅ Test with `test_mcp_server-client "test"`
...
This setup guide should get you productive with Test MCP Server quickly!