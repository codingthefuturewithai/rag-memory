"""Configuration management page for Test MCP Server Admin UI
...
def render_configuration_form(config):
    ...
    with st.form("config_form"):
        ...
        with col1:
            ...
            server_name = st.text_input("Server Name", value=server_config.get("name", "Test MCP Server"))
...
def handle_save_config(server_name, server_port, log_level, log_retention, current_config):
    ...
    new_config = {
        "server": {
            "name": server_name,
            "description": current_config.get("server", {}).get("description", "Test MCP Server - MCP server with tools and integrations"),
            "port": server_port,
            "log_level": log_level,
...
def handle_export_config(config):
    ...
    st.download_button(
        ...
        data=config_json,
        file_name="test_mcp_server_config.json",
...
def render_environment_variables():
    ...
    env_vars = {
        "TEST_MCP_SERVER_LOG_LEVEL": "INFO",
        "TEST_MCP_SERVER_PORT": "3001",
        "TEST_MCP_SERVER_CONFIG_PATH": system_info.get("config_path", "~/.config/test_mcp_server"),
        "TEST_MCP_SERVER_LOG_PATH": system_info.get("log_path", "~/.local/share/test_mcp_server"),
...
def main():
    ...
    # Page header
    st.title("⚙️ Test MCP Server Configuration")
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=config_yaml,
            file_name="test_mcp_server_config.yaml",
