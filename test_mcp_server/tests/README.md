# Testing Guide for Test MCP Server
...
uv run pytest tests/ --cov=test_mcp_server --cov-report=html
...
import pytest
from test_mcp_server.tools.my_tools import my_new_tool
...
uv run pytest tests/ --cov=test_mcp_server --cov-fail-under=40

# Generate coverage report for CI artifacts
uv run pytest tests/ --cov=test_mcp_server --cov-report=xml
```
...
4. Check coverage hasn't decreased: `uv run pytest tests/ --cov=test_mcp_server`
