...
```python
from test_mcp_server.log_system.destinations.factory import create_destination
...
2. **Register tool in server:**
   ```python
   # server/app.py
   from test_mcp_server.tools.my_tool import my_tool
...
# Check formatting
black --check test_mcp_server/
isort --check-only test_mcp_server/
...
# View logs
tail -f ~/.local/state/mcp-servers/logs/test_mcp_server.log  # Linux
tail -f ~/Library/Logs/mcp-servers/test_mcp_server.log       # macOS
...
- Use absolute imports: `from test_mcp_server.tools import my_tool`
